...
- **Opens start returning `503` after the monitor has been idle a while** → some ONEX monitors run
  an old firmware whose cloud registration goes **stale after a few idle minutes** (the app and the
  monitor's own status still say it's registered, but the PBX can no longer reach it). This hits the
  **official app** too. A power-cycle forces a fresh `REGISTER` and it works again. It's a
  monitor/PBX-side issue, not this bridge.
